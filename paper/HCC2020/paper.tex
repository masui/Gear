...
users can enjoy videos, musics, and other Web pages
on the browser with the Gear interface.

Serencast uses the ``\tsf{Scrapbox.io}'' wiki service\footnote{
...
} system where users can share and edit texts on the Web.
A user can create a Scrapbox ``project'', and create pages
...
\caption{An example of a Scrapbox project.}
\label{exampleproject}
\end{figure}

\begin{figure}[H]
\centerline{\includegraphics[width=80mm,bb=0 0 1606 1200]{figures/9e867c325bafc415bd0870c1717fbaf7.png}}
\caption{An example Scrapbox page.}
\label{examplepage}
\end{figure}

The page is based on plain text, and users can use special markup tags
like ``\texttt{[}'' and ``\texttt{]}'' to display bold texts, icons, URL links,
and links to other pages.
In this example, a URL link to \textsf{http://gyazo.com} is used
just like \textsf{A} tags in HTML.

{\scriptsize
\begin{verbatim}
Features
(The [Check.icon] means we believe this feature is unique \
 toScrapbox among competing services.)
...
Figure \ref{movielist} shows a Scrapbox page listing movies.
...
The raw text of this page looks like this.
...
When we access the Scrapbox page from Serencast,
``Gran Trino'' from Netflix automatically starts in the right side of the screen,
just like a TV program starts when we rotate tha channel dial of an old TV.
...
We can add more data by hierarchically proiding the contents of Scrapbox pages.
...
musics titles by Weather Report and Wes Montgomery are shown.
For example, 
``Black Market'' is the first music title in the album ``8:30'' of Weather Report, and
the tune ``Scarlet Woman'' follows ``Black Market''.
``Wes Montgomery'' is listed in the same layer as ``Weather Report'', and
the album ``Tequilla'' is listed below the level.
...
The title of this Scrapbox page is ``Music'', and 
...
Many pages (Music, Movies, etc.) are listed below the Top page, and
the pages are treated as children pages of the Top page.
By constructing data hierarchically like this,
We can build a very large hierarchical database for watching movies, videos, etc.
As many as 100,000 pages can be put on a hierarchical Scrapbox pages, and
...
Since anybody can edit the Scrapbox page and construct the hierarchical data,
people can enjoy creating his own data by editing his Scrapbox pages.
Also, people who want to distribute their works,
they can create their Scrapbox page
and give it to others so that other people can include the page and
enjoy it by using Gear.

\section{DISCUSSIONS}

\subsection{Input devices for Gear}

Since only two switches are required,
...
We can use two arrow keys on a computer for using Serencast.
