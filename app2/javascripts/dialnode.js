...
if(showContents){
    // gui = require('nw.gui');
    // win = gui.Window.open('https://github.com', {
    // });

    //win = window.open("contents.html");  // YouTube, クックパッド等がiframeで表示できないので別ウィンドウを開く
    //win.moveTo(screen.width * 25 / 100,0);
    //win.resizeTo(screen.width * 73 / 100,screen.height);
...
$(function() { // 最初に呼ばれるjQueryのready関数
    ...
    iframe.css('width','400pt');
    iframe.css('height','400pt');
    iframe.css('top','40pt');
    iframe.css('left','400pt');
    iframe.css('background-color','#ffffff');
    iframe.attr('src',"http://pitecan.com");
...
var display = function(newNodeList){ // calc()で計算したリストを表示
    ...
    if(nodeList[0].url && showContents){ // 別ウィンドウにコンテンツ表示
    	...
    	// var url = win.location.href = nodeList[0].url;
	// //if(url.match(/geta6/)){
	...
	//iframe = win.document.getElementById('iframe');
	//iframe.src = url;
